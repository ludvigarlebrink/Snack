...
namespace snack
{
SketchTextEditor::SketchTextEditor()
	: mLineSpacing(1.0f)
	, mUndoIndex(0)
	, mTabSize(4)
	, mOverwrite(false)
...
void SketchTextEditor::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
{
...
void SketchTextEditor::SetPalette(const Palette & aValue)
{
	mPaletteBase = aValue;
}

int SketchTextEditor::AppendBuffer(std::string& aBuffer, char chr, int aIndex)
{
	if (chr != '\t')
	{
...
std::string SketchTextEditor::GetText(const Coordinates& aStart, const Coordinates& aEnd) const
{
	std::string result;
...
SketchTextEditor::Coordinates SketchTextEditor::GetActualCursorCoordinates() const
{
	return SanitizeCoordinates(mState.mCursorPosition);
}

SketchTextEditor::Coordinates SketchTextEditor::SanitizeCoordinates(const Coordinates& aValue) const
{
...
void SketchTextEditor::Advance(Coordinates& aCoordinates) const
{
...
void SketchTextEditor::DeleteRange(const Coordinates& aStart, const Coordinates& aEnd)
{
	assert(aEnd >= aStart);
	assert(!mReadOnly);
...
int SketchTextEditor::InsertTextAt(Coordinates& aWhere, const char* aValue)
{
	assert(!mReadOnly);
...
void SketchTextEditor::AddUndo(UndoRecord& aValue)
{
	assert(!mReadOnly);
...
SketchTextEditor::Coordinates SketchTextEditor::ScreenPosToCoordinates(const glm::vec2& aPosition) const
{
...
SketchTextEditor::Coordinates SketchTextEditor::FindWordStart(const Coordinates& aFrom) const
{
...
SketchTextEditor::Coordinates SketchTextEditor::FindWordEnd(const Coordinates& aFrom) const
{
	Coordinates at = aFrom;
    if (at.mLine >= (int)mLines.size())
    {
...
bool SketchTextEditor::IsOnWordBoundary(const Coordinates & aAt) const
{
...
void SketchTextEditor::RemoveLine(int aStart, int aEnd)
{
	assert(!mReadOnly);
	assert(aEnd >= aStart);
...
void SketchTextEditor::RemoveLine(int aIndex)
{
	assert(!mReadOnly);
...
SketchTextEditor::Line& SketchTextEditor::InsertLine(int aIndex)
{
	assert(!mReadOnly);
...
std::string SketchTextEditor::GetWordUnderCursor() const
{
	auto c = GetCursorPosition();
	return GetWordAt(c);
}

std::string SketchTextEditor::GetWordAt(const Coordinates & aCoords) const
{
...
ImU32 SketchTextEditor::GetGlyphColor(const Glyph & aGlyph) const
{
	if (aGlyph.mComment)
...
void SketchTextEditor::HandleKeyboardInputs()
{
...
void SketchTextEditor::HandleMouseInputs()
{
	ImGuiIO& io = ImGui::GetIO();
	auto shift = io.KeyShift;
	auto ctrl = io.ConfigMacOSXBehaviors ? io.KeySuper : io.KeyCtrl;
		...
		if (!shift && !alt)
		{
			...
			*/
			...
			if (tripleClick)
			{
				if (!ctrl)
				{
					mState.mCursorPosition = mInteractiveStart = mInteractiveEnd = SanitizeCoordinates(ScreenPosToCoordinates(glm::vec2(ImGui::GetMousePos().x, ImGui::GetMousePos().y)));
			...
			*/
			...
			else if (ImGui::IsMouseDragging(0) && ImGui::IsMouseDown(0))
			{
				...
				mState.mCursorPosition = mInteractiveEnd = SanitizeCoordinates(ScreenPosToCoordinates(glm::vec2(ImGui::GetMousePos().x, ImGui::GetMousePos().y)));
				SetSelection(mInteractiveStart, mInteractiveEnd, mSelectionMode);
			}
		}
	}
...
void SketchTextEditor::Render()
{
	/* Compute mCharAdvance regarding to scaled font size (Ctrl + mouse wheel)*/
	const float fontSize = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, "#", nullptr, nullptr).x;
	ImVec2 imCharAdvance = ImVec2(fontSize, ImGui::GetTextLineHeightWithSpacing() * mLineSpacing);
    mCharAdvance = glm::vec2(imCharAdvance.x, imCharAdvance.y);
	...
	if (!mLines.empty())
	{
		...
		{
			auto id = GetWordAt(ScreenPosToCoordinates(glm::vec2(ImGui::GetMousePos().x, ImGui::GetMousePos().y)));
			if (!id.empty())
			{
...
void SketchTextEditor::Render(const char* aTitle, const glm::vec2& aSize, bool aBorder)
{
	mWithinRender = true;
	mTextChanged = false;
	...
	ImGui::BeginChild(aTitle, ImVec2(aSize.x, aSize.y), aBorder, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_AlwaysHorizontalScrollbar | ImGuiWindowFlags_NoMove);
...
void SketchTextEditor::SetText(const std::string& aText)
{
	mLines.clear();
...
void SketchTextEditor::SetTextLines(const std::vector<std::string>& aLines)
{
	mLines.clear();
...
void SketchTextEditor::EnterCharacter(Char aChar, bool aShift)
{
	assert(!mReadOnly);
	...
	if (HasSelection())
	{
		if (aChar == '\t')
		{
			...
			for (int i = start.mLine; i <= end.mLine; i++)
			{
				...
				{
					line.insert(line.begin(), Glyph('\t', SketchTextEditor::PaletteIndex::Background));
					if (i == end.mLine)
						++end.mColumn;
					modified = true;
				}
...
void SketchTextEditor::SetReadOnly(bool aValue)
{
	mReadOnly = aValue;
}

void SketchTextEditor::SetCursorPosition(const Coordinates& aPosition)
{
...
void SketchTextEditor::SetSelectionStart(const Coordinates& aPosition)
{
...
void SketchTextEditor::SetSelectionEnd(const Coordinates& aPosition)
{
...
void SketchTextEditor::SetSelection(const Coordinates& aStart, const Coordinates& aEnd, SelectionMode aMode)
{
	...
	switch (aMode)
	{
	case SketchTextEditor::SelectionMode::Normal:
		break;
	case SketchTextEditor::SelectionMode::Word:
	{
	...
	case SketchTextEditor::SelectionMode::Line:
	{
...
void SketchTextEditor::InsertText(const std::string & aValue)
{
	InsertText(aValue.c_str());
}

void SketchTextEditor::InsertText(const char * aValue)
{
	if (aValue == nullptr)
		return;
...
void SketchTextEditor::DeleteSelection()
{
...
void SketchTextEditor::MoveUp(int aAmount, bool aSelect)
{
...
void SketchTextEditor::MoveDown(int aAmount, bool aSelect)
{
...
void SketchTextEditor::MoveLeft(int aAmount, bool aSelect, bool aWordMode)
{
	if (mLines.empty())
		return;
...
void SketchTextEditor::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	auto oldPos = mState.mCursorPosition;
...
void SketchTextEditor::MoveTop(bool aSelect)
{
	auto oldPos = mState.mCursorPosition;
...
void SketchTextEditor::SketchTextEditor::MoveBottom(bool aSelect)
{
...
void SketchTextEditor::MoveHome(bool aSelect)
{
	auto oldPos = mState.mCursorPosition;
...
void SketchTextEditor::MoveEnd(bool aSelect)
{
	auto oldPos = mState.mCursorPosition;
...
void SketchTextEditor::BackSpace()
{
	assert(!mReadOnly);
...
void SketchTextEditor::SelectWordUnderCursor()
{
	auto c = GetCursorPosition();
	SetSelection(FindWordStart(c), FindWordEnd(c));
}

void SketchTextEditor::SelectAll()
{
	SetSelection(Coordinates(0, 0), Coordinates((int)mLines.size(), 0));
}

bool SketchTextEditor::HasSelection() const
{
	return mState.mSelectionEnd > mState.mSelectionStart;
}

void SketchTextEditor::Copy()
{
	if (HasSelection())
	{
...
bool SketchTextEditor::CanUndo() const
{
	return mUndoIndex > 0;
}

bool SketchTextEditor::CanRedo() const
{
	return mUndoIndex < (int)mUndoBuffer.size();
}

void SketchTextEditor::Undo(int aSteps)
{
	while (CanUndo() && aSteps-- > 0)
		mUndoBuffer[--mUndoIndex].Undo(this);
}

void SketchTextEditor::Redo(int aSteps)
{
	while (CanRedo() && aSteps-- > 0)
		mUndoBuffer[mUndoIndex++].Redo(this);
}

const SketchTextEditor::Palette & SketchTextEditor::GetDarkPalette()
{
...
const SketchTextEditor::Palette & SketchTextEditor::GetLightPalette()
{
	const static Palette p = { {
		0xff7f7f7f,	// None
		0xffff0c06,	// Keyword	
...
const SketchTextEditor::Palette & SketchTextEditor::GetRetroBluePalette()
{
	const static Palette p = { {
		0xff00ffff,	// None
		0xffffff00,	// Keyword	
...
std::string SketchTextEditor::GetText() const
{
	return GetText(Coordinates(), Coordinates((int)mLines.size(), 0));
}

std::vector<std::string> SketchTextEditor::GetTextLines() const
{
...
std::string SketchTextEditor::GetSelectedText() const
{
	return GetText(mState.mSelectionStart, mState.mSelectionEnd);
}

std::string SketchTextEditor::GetCurrentLineText()const
{
...
void SketchTextEditor::ProcessInputs()
{
}

void SketchTextEditor::Colorize(int aFromLine, int aLines)
{
...
void SketchTextEditor::ColorizeRange(int aFromLine, int aToLine)
{
...
void SketchTextEditor::ColorizeInternal()
{
	if (mLines.empty())
		return;
...
float SketchTextEditor::TextDistanceToLineStart(const Coordinates& aFrom) const
{
...
void SketchTextEditor::EnsureCursorVisible()
{
	if (!mWithinRender)
	{
...
int SketchTextEditor::GetPageSize() const
{
	auto height = ImGui::GetWindowHeight() - 20.0f;
	return (int)floor(height / mCharAdvance.y);
}

SketchTextEditor::UndoRecord::UndoRecord(
...
void SketchTextEditor::UndoRecord::Undo(SketchTextEditor * aEditor)
{
	if (!mAdded.empty())
	{
...
void SketchTextEditor::UndoRecord::Redo(SketchTextEditor * aEditor)
{
	if (!mRemoved.empty())
	{
...
const SketchTextEditor::LanguageDefinition& SketchTextEditor::LanguageDefinition::CPlusPlus()
{
...
const SketchTextEditor::LanguageDefinition& SketchTextEditor::LanguageDefinition::HLSL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const SketchTextEditor::LanguageDefinition& SketchTextEditor::LanguageDefinition::GLSL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const SketchTextEditor::LanguageDefinition& SketchTextEditor::LanguageDefinition::C()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const SketchTextEditor::LanguageDefinition& SketchTextEditor::LanguageDefinition::SQL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const SketchTextEditor::LanguageDefinition& SketchTextEditor::LanguageDefinition::AngelScript()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const SketchTextEditor::LanguageDefinition& SketchTextEditor::LanguageDefinition::Lua()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
